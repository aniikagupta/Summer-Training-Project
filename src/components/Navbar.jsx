...
const Navbar = () => {
  ...
  // one list used for both the desktop menu and the mobile menu
  const links = [
    { path: '/', label: 'Home' },
  ...
  // the search bar only shows on the collection page, so go there first
  ...
  return (
    <div className='flex justify-between items-center py-5 font-medium'>
      ...
      <Link to='/' className='text-xl font-semibold tracking-widest text-gray-900'>SHOPIFY</Link>

      {/* Desktop menu - NavLink gives us isActive so we can underline the current page */}
      <nav className="hidden sm:flex gap-6 text-sm">
        {links.map((link) => (
          <NavLink
            ...
            to={link.path}
            className={({ isActive }) =>
              `pb-1 border-b-2 ${isActive ? 'text-black border-black' : 'text-gray-500 border-transparent hover:text-black'}`
            }
          >
            {link.label}
          </NavLink>
      ...
      <div className="flex items-center gap-3 text-sm">
        <img onClick={handleSearchClick} src={search} className='w-7 cursor-pointer' alt="search" />
        ...
        <img onClick={() => setVisible(true)} src={menu} className='w-7 cursor-pointer sm:hidden' alt="menu" />
      </div>

      {/* Mobile menu - fixed + z-50 so it covers the page instead of sitting behind it */}
      <div className={`fixed top-0 left-0 z-50 h-full bg-white overflow-hidden transition-all ${visible ? 'w-full' : 'w-0'}`}>
        <div className="flex flex-col text-gray-500">
          ...
          {links.map((link) => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `py-3 pl-6 border-b ${isActive ? 'text-black' : 'hover:text-black'}`
              }
            >
              {link.label}
            </NavLink>
